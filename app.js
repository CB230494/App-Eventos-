(() => {
  ...
  function addBasilicaMarker() {
    ...
    basilicaMarker=new maplibregl.Marker({element:el, anchor:'bottom'})
  ...
  function renderServiceMarkers() {
    ...
    data.services
      ...
      .forEach(service => {
        ...
        el.type='button';
        el.innerHTML=`<span>${meta.icon}</span>`;
  ...
  function wireUi() {
    ...
    $('northBtn').addEventListener('click', () => map.easeTo({bearing:0,pitch:navigation.active?55:0,duration:600}));
  ...
  async function toggleMapFocusMode() {
    ...
    setTimeout(()=>{
      map?.resize();
      if (navigation.active) followUserInNavigation(true);
    },180);
  }
  ...
  async function updateBasilicaRouteSummary() {
    ...
    try {
      ...
      console.warn(err);
      const km=haversineKm(userLocation.lat,userLocation.lng,basilica.lat,basilica.lng);
      $('basilicaDistance').textContent=formatDistance(km);
      $('distanceNote').textContent='No se pudo consultar la ruta peatonal; se muestra referencia aproximada.';
    }
  }
  ...
  async function updateNearestByWalkingRoute(force=false) {
    ...
    try {
      ...
      const ranked=eligible.map((s,i)=>({service:s,duration:matrix.durations?.[0]?.[i],distance:matrix.distances?.[0]?.[i]}))
        .filter(x => Number.isFinite(x.duration) && Number.isFinite(x.distance))
        .sort((a,b)=>a.distance-b.distance || a.duration-b.duration);
      if (!ranked.length) throw new Error('No hay recorridos peatonales disponibles');
      nearestService=ranked[0].service;
      nearestRouteInfo={duration:ranked[0].duration,distance:ranked[0].distance};
      renderNearestResult();
    } catch (err) {
      console.warn('Matrix routing:',err);
      const fallback=eligible.map(s=>({...s,distance:haversineKm(userLocation.lat,userLocation.lng,s.lat,s.lng)})).sort((a,b)=>a.distance-b.distance)[0];
      nearestService=fallback;
      nearestRouteInfo=null;
      renderNearestResult(true);
    } finally {
  ...
  function renderNearestResult(fallback=false) {
    ...
    $('nearestResult').innerHTML=`
      <div class="nearest-icon service-color-${nearestService.type}">${meta.icon}</div>
      <div class="nearest-copy">${routeLine}<p>${fallback?'Punto aproximado por proximidad geográfica.':'Es el punto con menor recorrido peatonal de esta categoría.'}</p></div>
  ...
  async function fetchWalkingMatrix(origin,destinations) {
    ...
    if (!res.ok) throw new Error(`Routing table HTTP ${res.status}`);
  ...
  async function fetchWalkingRoute(origin,destination,withSteps=true) {
    ...
    const res=await fetch(url,{headers:{Accept:'application/json'}});
    if (!res.ok) throw new Error(`Routing route HTTP ${res.status}`);
    const json=await res.json();
  ...
  function followUserInNavigation(force=false) {
    if (!navigation.active || !userLocation) return;
    const bearing=getUserBearing();
    map.easeTo({
  ...
  function updateNavigationProgressFromGps() {
    ...
    const finalDist=haversineMeters(userLocation.lat,userLocation.lng,navigation.destination.lat,navigation.destination.lng);
    if (finalDist < 30) {
  ...
  function recenterOnUser() {
    ...
    if (navigation.active) followUserInNavigation(true);
    else map.easeTo({center:[userLocation.lng,userLocation.lat],zoom:15.4,pitch:0,bearing:0,duration:700});
  }
