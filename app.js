(() => {
  ...
  function finalizeRoute(route,destination) {
    ...
    const gap=haversineMeters(last[1],last[0],destination.lat,destination.lng);
    // El motor puede ajustar el destino a la acera/calle. Extendemos visualmente la ruta
    // hasta el marcador exacto del puesto para que la llegada coincida con el icono.
    if (gap>2) {
      geometry.coordinates.push([destination.lng,destination.lat]);
      route.distance=Number(route.distance||0)+gap;
      route.duration=Number(route.duration||0)+(gap/1.25);
    }
    return {...route,geometry,finalConnectorMeters:gap};
  }
